...
export default function CourtDetailsView({ pm }: propsType) {
  return (
    ...
    >
      ...
      <Text style={styles.court_name}>{pm.courtDetails.name}</Text>

      <View style={styles.location}>
        <Location stroke="black" width={20} height={20} />
        <Text style={styles.court_location}>{pm.courtDetails.location}</Text>
      </View>

      <Text style={styles.court_data}>{pm.courtDetails.description}</Text>
